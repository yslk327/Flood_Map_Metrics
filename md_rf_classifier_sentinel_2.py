# -*- coding: utf-8 -*-
"""MD_RF_classifier_Sentinel-2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1p50ULst5CsJLNi5bj1DM7Ae-oRK-SkL-
...
water_data = ee.FeatureCollection("projects/ee-yassineloukili/assets/water_data2_tetouan_300")  #Training sample water Tetouan_Morocco
...
water_data = ee.FeatureCollection("projects/ee-yassineloukili/assets/water_data_Senegal_300")  # Training sample water Matam_Senegal
...
trained_MD = classifier_MD.train(features=Sample_LS, classProperty="land_class", inputProperties=bandNamesS2)
...
trained_RF = classifier_RF.train(features=Sample_LS, classProperty="land_class", inputProperties=bandNamesS2)
...
'''
# Compute uncertainty: 1 - 2 * |P - 0.5|
uncertainty = combined_prob.subtract(0.6).abs().multiply(-2).add(1).rename('Uncertainty')
# Filter medium and high uncertainty only
low_thresh = 0.33
high_thresh = 0.66
uncertainty_masked = uncertainty.updateMask(uncertainty.gt(low_thresh))
# Full Uncertainty (Medium, High)
uncertainty_vis = uncertainty_masked.visualize(min=low_thresh,max=1,palette=['#FFFF00', '#FF0000'])
# Filter low medium and high uncertainty
flood_mask = ee.Image.constant(1).clip(water_extent_area).rename('flood_mask')
# Mask uncertainty to flooded areas only
uncertainty_in_flood =uncertainty.updateMask(flood_mask)
# Full Uncertainty (Low, Medium, High)
uncertainty_vis = uncertainty_in_flood.visualize(min=0,max=1,palette=['#00FF00', '#FFFF00', '#FF0000'])
'''
# Create RGB image
rgb_vis = image_before.select(['B4', 'B3', 'B2']).visualize(min=300, max=2500)
...
# Display in notebook
#print("🔗 Download your composite PNG map here:")
#print(url)
...
plt.ylabel("Latitude")
plt.title("SENTINEL-2 - Matam", fontsize=16)
#plt.title("SENTINEL-2 - Tetouan", fontsize=16)
#plt.title("SENTINEL-2 - MD", fontsize=16)
#plt.title("SENTINEL-2 - RF", fontsize=16)
#plt.title("SENTINEL-2 - RF & MD", fontsize=16)
#plt.title("PLEIADES - Tetouan", fontsize=16)
...
ax.annotate('N',
            xy=(0.95, 0.15),  # arrow base in axis fraction coordinates
            xytext=(0.95, 0.05),
